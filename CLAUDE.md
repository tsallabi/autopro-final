...
- **Email:** unified `sendEmail()` helper prefers **Resend** (`RESEND_API_KEY`) and falls back to **SMTP** via nodemailer (`mail.privateemail.com` for `info@autopro.ac`). Both paths must keep working — Resend is the primary because Render's free tier blocks outbound SMTP.
