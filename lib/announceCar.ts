 ...
 *   1. Internal message (in their inbox)
 *   2. Notification (bell icon)
 *   3. Email with the hero image, price, schedule, and a "View car" CTA
 *
 * Used by:
 *   - POST /api/admin/cars/:id/announce (manual trigger from AdminQuickTools)
 *   - POST /api/admin/cars/:id/approve-with-schedule (auto-trigger on approval)
 *
 * Runs async — schedules per-user sends in the background and returns
 * an immediate counter so the admin UI doesn't have to wait 30s for 1000 emails.
 *
 * No DB schema dependency. Uses existing helpers from AppContext.
 */
...
<body style="margin:0;padding:0;background:#f8fafc;font-family:Arial,sans-serif;">
  <div style="max-width:600px;margin:0 auto;padding:24px;">
    ...
    ${hero ? `<img src="${hero}" alt="${title}" style="width:100%;max-height:340px;object-fit:cover;border-radius:14px;margin:18px 0;display:block;">` : ''}
    <h2 style="color:#1e293b;margin:0 0 12px;font-size:22px;">${title}</h2>
...
function buildPlainMessage(car: any, siteUrl: string): { subject: string; content: string } {
  const title = `${car.year || ''} ${car.make || ''} ${car.model || ''}`.trim();
...
export function announceCarToAllUsers(
  ...
  const { subject, content } = buildPlainMessage(car, SITE_URL);
  const html = buildEmailHtml(car, SITE_URL);
  const carUrl = `/car-details/${car.id}`;
